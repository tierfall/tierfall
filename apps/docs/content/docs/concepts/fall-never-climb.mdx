...
When the router can't get a response from an adapter, it tries the next-cheapest one. It never tries the next-more-expensive one. This is _the_ design constraint — the whole library is named for it.
